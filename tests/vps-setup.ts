...
async function globalSetup(config: FullConfig): Promise<void> {
  ...
  try {
    ...
    if (!servicesResponse || typeof servicesResponse !== "object") {
      console.error(
        "[VPS Setup] Dashboard returned invalid response (not an object)",
      );
    ...
    if (!services || typeof services !== "object") {
      console.error(
        '[VPS Setup] Dashboard response missing "services" field or invalid type',
      );
    ...
    console.log(
      `[VPS Setup] Dashboard connected - ${serviceCount} services registered`,
    );
    ...
    console.log(
      `[VPS Setup] Ensuring core services: ${coreServices.join(", ")}`,
    );
    ...
    for (const serviceId of coreServices) {
      try {
        ...
        console.warn(
          `[VPS Setup] Could not start ${serviceId}: ${error.message}`,
        );
    ...
    try {
      ...
      if (vramStatus?.gpu) {
        ...
        console.warn(
          "[VPS Setup] VRAM status returned unexpected structure, skipping VRAM log",
        );
      }
    } catch (vramError: any) {
      console.warn(
        `[VPS Setup] Could not fetch VRAM status: ${vramError.message}`,
      );
    }
