...
test.describe("Open WebUI E2E Chat Interaction", () => {
  test("loads dashboard, opens WebUI, sends message, and captures screenshot", async ({
    ...
    if (!isCardVisible) {
      console.log(
        "[Test] Open WebUI service card not found on dashboard, navigating directly",
      );
    ...
    try {
      ...
      console.log(
        "[Test] Open button click failed, trying alternative approach",
      );
      // Try clicking any link/button that opens the service
      const openLink = page
        .locator(`a[href*="3000"], a[href*="openwebui"]`)
        .first();
    ...
    try {
      ...
      console.log(
        "[Test] New tab did not open, checking current page or navigating directly",
      );
