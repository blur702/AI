...
test.describe('VRAM Management Integration', () => {
  test.describe('VRAM Status Monitoring', () => {
    test('VRAM endpoint returns complete usage data', async ({ dashboardAPI }) => {
  ...
  test.describe('Model Memory Manager', () => {
    ...
    test('loaded models count matches or is less than available', async ({ dashboardAPI }) => {
  ...
  test.describe('VRAM and Model Correlation', () => {
    test('VRAM status reflects model loading', async ({ dashboardAPI }) => {
      ...
      console.log(`VRAM Used: ${vramStatus.gpu.used_mb} MB / ${vramStatus.gpu.total_mb} MB`);
  ...
  test.describe('Model Unload Functionality', () => {
    test('can request model unload', async ({ dashboardAPI }) => {
      ...
      try {
        ...
        console.log(`Unload returned error (may be expected): ${error.message || error}`);
      }
    });

    test('invalid model unload returns appropriate error', async ({ dashboardAPI }) => {
  ...
  test.describe('Concurrent Operations', () => {
    test('multiple VRAM status requests work correctly', async ({ dashboardAPI }) => {
      // Fire multiple requests concurrently
      const requests = Array(5).fill(null).map(() => dashboardAPI.getVRAMStatus());
    ...
    test('VRAM and model list requests work concurrently', async ({ dashboardAPI }) => {
