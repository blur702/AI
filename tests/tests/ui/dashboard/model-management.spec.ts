...
test.describe("Dashboard model management", () => {
  ...
  test.describe("Model Memory Management", () => {
    test("can check which models are consuming VRAM", async ({
      dashboardAPI,
    }) => {
      ...
      console.log(
        `GPU Memory: ${vramStatus.gpu.used_mb}/${vramStatus.gpu.total_mb} MB`,
      );
    ...
    test("unloading models should free VRAM", async ({ dashboardAPI }) => {
      ...
      if (loadedBefore.length > 0) {
        ...
        try {
          ...
          expect(vramAfter.gpu.used_mb).toBeLessThanOrEqual(
            vramBefore.gpu.used_mb + 100,
          ); // 100MB tolerance
        } catch (error) {
