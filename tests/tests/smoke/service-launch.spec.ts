...
test.describe('Service Tab Launch Tests', () => {
  for (const service of SERVICES) {
    test(`should open ${service.name} in new tab`, async ({ context, page }) => {
      ...
      try {
        ...
        if (status === 401) {
          console.log(`${service.name}: Requires authentication (401) - service is reachable`);
        ...
        if (error.message?.includes('timeout') || error.message?.includes('ECONNREFUSED')) {
          test.skip(true, `${service.name} is not accessible: ${error.message}`);
          return;
        }
        throw error;
      } finally {
...
test.describe('Dashboard Service Elements', () => {
  test('should display service cards with proper structure', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });

    // Look for service cards or service list
    const serviceElements = page.locator('[class*="service"], [class*="card"], [data-service]');
  ...
  test('should display service status indicators', async ({ page }) => {
    ...
    const statusIndicators = page.locator('[class*="status"], [class*="indicator"], [class*="running"], [class*="stopped"]');
...
test.describe('Service Start/Stop Interaction', () => {
  ...
  test('should toggle a service and verify status change', async ({ page }) => {
    ...
    const serviceCard = page.locator('[data-service="n8n"], [class*="service"][class*="n8n"]').first();
    let targetCard = serviceCard;

    if (await serviceCard.count() === 0) {
      // Fallback: find any service card with a control button
      const anyServiceCard = page.locator('[class*="service"], [class*="card"]').filter({
        has: page.locator('button[class*="start"], button[class*="stop"], button[class*="toggle"]')
      }).first();
    ...
    const statusIndicator = targetCard.locator('[class*="status"], [class*="indicator"]').first();
    ...
    const toggleButton = targetCard.locator(
    ...
    console.log(`Initial status - class: "${initialState.classList}", text: "${initialState.textContent}"`);
    ...
    while (Date.now() - startTime < maxWaitTime) {
      ...
      if (classChanged || textChanged) {
        ...
        console.log(`Final status - class: "${finalState.classList}", text: "${finalState.textContent}"`);
        break;
      }
    }
    ...
    if (!statusChanged) {
      console.warn('Status did not change within timeout - service may be unresponsive or already in target state');
    ...
    const isTransitioning = finalState.textContent.includes('starting') ||
...
test.describe('External Accessibility', () => {
  test('should verify nginx proxy paths are accessible', async ({ request }) => {
    const proxyPaths = [
      { path: '/', name: 'Dashboard' },
...
test.describe('Visual Regression Baseline', () => {
  test('should capture dashboard screenshot for visual comparison', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });
  ...
  for (const service of SERVICES.slice(0, 3)) { // Limit to first 3 for baseline
    test(`should capture ${service.name} screenshot for visual comparison`, async ({ context }) => {
      const newPage = await context.newPage();
      const serviceUrl = `${BASE_URL}${service.proxyPath}`;
      ...
      try {
        ...
        if (error.message?.includes('timeout')) {
          test.skip(true, `${service.name} not accessible for baseline capture`);
        } else {
          throw error;
        }
      } finally {
