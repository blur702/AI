...
test.describe('API Gateway Authentication', () => {
  test.describe('API Key Creation', () => {
    test('creates API key with valid name', async ({ gatewayAPI, cleanupAPIKeys }) => {
    ...
    test('creates multiple API keys with different names', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');
  ...
  test.describe('API Key Listing', () => {
    test('lists all API keys without exposing key values', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');
    ...
    test('shows correct is_active status', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');

      // Create a key
      const createResponse = await gatewayAPI.createAPIKey('active-status-test');
      ...
      let keyInfo = listResponse.data.keys.find((k: any) => k.name === 'active-status-test');
      ...
      keyInfo = listResponse.data.keys.find((k: any) => k.name === 'active-status-test');
      expect(keyInfo).toBeDefined();
  ...
  test.describe('API Key Deactivation', () => {
    test('deactivates existing API key', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');
      ...
      const listResponse = await gatewayAPI.listAPIKeys();
      const keyInfo = listResponse.data.keys.find((k: any) => k.name === 'deactivate-test');
      expect(keyInfo).toBeDefined();
      expect(keyInfo.is_active).toBe(false);
    });

    test('deactivation is idempotent', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');
    ...
    test('deactivating non-existent key succeeds', async ({ gatewayAPI }) => {
      ...
      const response = await gatewayAPI.deactivateAPIKey('non-existent-key-12345');
      expect(response.success).toBe(true);
    });
  });

  test.describe('Middleware - Valid API Key', () => {
    test('allows access to protected endpoint with valid API key', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');

      // Create a valid API key
      const createResponse = await gatewayAPI.createAPIKey('protected-endpoint-test');
      const apiKey = createResponse.data.key;
      cleanupAPIKeys.registerAPIKey(apiKey);
    ...
    test('updates last_used_at timestamp on API key usage', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');
      ...
      let listResponse = await gatewayAPI.listAPIKeys();
      let keyInfo = listResponse.data.keys.find((k: any) => k.name === 'timestamp-test');
      expect(keyInfo).toBeDefined();
      ...
      try {
        ...
        if (error instanceof APIError && error.status === 401) {
          throw new Error('API key should have been accepted but got 401 Unauthorized');
        }
      ...
      listResponse = await gatewayAPI.listAPIKeys();
      keyInfo = listResponse.data.keys.find((k: any) => k.name === 'timestamp-test');
      expect(keyInfo).toBeDefined();
  ...
  test.describe('Middleware - Invalid API Key', () => {
    test('rejects request with missing API key header', async ({ gatewayAPI }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');
    ...
    test('rejects request with deactivated API key', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');

      // Create and deactivate a key
      const createResponse = await gatewayAPI.createAPIKey('deactivated-key-test');
      const apiKey = createResponse.data.key;
      cleanupAPIKeys.registerAPIKey(apiKey);
      ...
      try {
        ...
        expect(error.data?.success).toBe(false);
        expect(error.data?.error?.code).toBe('INVALID_API_KEY');
        expect(error.data?.error?.message).toContain('Invalid or inactive API key');
      }
      expect(didThrow).toBe(true);
    });
  });
  ...
  test.describe('Error Handling', () => {
    test('handles gateway unavailable gracefully', async ({ gatewayAPI }) => {
      ...
      try {
        ...
        if (error.code === 'ECONNREFUSED' || error.message?.includes('ECONNREFUSED')) {
          test.skip(true, 'API Gateway is not running - this is expected behavior');
          return;
        }
    ...
    test('validates response format for all endpoints', async ({ gatewayAPI, cleanupAPIKeys }) => {
      const isAvailable = await checkGatewayAvailable(gatewayAPI);
      test.skip(!isAvailable, 'API Gateway is not running (port 1301)');
      ...
      const deactivateResponse = await gatewayAPI.deactivateAPIKey(createResponse.data.key);
