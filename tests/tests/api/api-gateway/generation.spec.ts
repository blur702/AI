...
test.describe('API Gateway generation endpoints', () => {
  ...
  test.skip('image generation returns a job or result', async ({ gatewayAPI, testData }) => {
    const prompt = testData.prompts.image[0];
    const response = await gatewayAPI.generateImage({ prompt: prompt.prompt });
    expect(response.success).toBe(true);
  });

  test.skip('music generation returns a job or result', async ({ gatewayAPI, testData }) => {
