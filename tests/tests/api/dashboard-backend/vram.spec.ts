...
test.describe('Dashboard Backend VRAM API', () => {
  test.describe('GET /api/vram/status', () => {
    test('returns VRAM status with expected GPU schema', async ({ dashboardAPI }) => {
    ...
    test('processes have expected schema when present', async ({ dashboardAPI }) => {
      const status = await dashboardAPI.getVRAMStatus();
    ...
    test('used + free memory approximately equals total', async ({ dashboardAPI }) => {
      const status = await dashboardAPI.getVRAMStatus();

      const calculatedTotal = status.gpu.used_mb + status.gpu.free_mb;
      // Allow 5% tolerance for rounding
      const tolerance = status.gpu.total_mb * 0.05;
      expect(Math.abs(calculatedTotal - status.gpu.total_mb)).toBeLessThan(tolerance);
    });

    test('multiple requests return consistent GPU name', async ({ dashboardAPI }) => {
