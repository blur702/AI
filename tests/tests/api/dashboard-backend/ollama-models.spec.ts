...
test.describe('Dashboard Backend Ollama Models API', () => {
  test.describe('GET /api/models/ollama/list', () => {
    test('returns list of available Ollama models', async ({ dashboardAPI }) => {
    ...
    test('returns consistent results on multiple calls', async ({ dashboardAPI }) => {
      ...
      expect(response1.models.map(m => m.name).sort())
        .toEqual(response2.models.map(m => m.name).sort());
    });
  });
  ...
  test.describe('GET /api/models/ollama/loaded', () => {
    ...
    test('loaded models are subset of available models', async ({ dashboardAPI }) => {
  ...
  test.describe('POST /api/models/ollama/unload', () => {
    ...
    test('handles unload of non-existent model gracefully', async ({ dashboardAPI }) => {
      try {
        const result = await dashboardAPI.unloadModel('nonexistent-model-xyz-123');
  ...
  test.describe('Model load/unload integration', () => {
    ...
    test('unloading model removes it from loaded list', async ({ dashboardAPI }) => {
