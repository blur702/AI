 ...
 */
export const EMBEDDING_HOST_SERVICES = ["ollama", "weaviate"];
...
export function getVPSConfig(): VPSConfig {
  ...
  const config: VPSConfig = {
    ...
    serviceStartTimeout: parseEnvNumber(
      process.env.SERVICE_START_TIMEOUT,
      60000,
    ),
    serviceHealthInterval: parseEnvNumber(
      process.env.SERVICE_HEALTH_INTERVAL,
      5000,
    ),
...
export function logVPSEnvironment(): void {
  ...
  console.log(
    `Preserve Embeddings: ${config.preserveEmbeddingModels.join(", ")}`,
  );
