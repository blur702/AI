import {
  DashboardAPIClient,
  ServiceInfo,
} from "../api-clients/DashboardAPIClient";
import {
...
export class ServiceOrchestrator {
  ...
  constructor(
    ...
    config: Partial<ServiceOrchestrationConfig> = {},
  ) {
    this.client = client;
  ...
  async ensureServiceRunning(serviceId: string): Promise<void> {
    console.log(
      `[ServiceOrchestrator] Ensuring service '${serviceId}' is running...`,
    );
    ...
    while (attempts < this.config.maxRetries) {
      try {
        ...
        if (isRunning) {
          console.log(
            `[ServiceOrchestrator] Service '${serviceId}' is already running`,
          );
          return;
        }

        console.log(
          `[ServiceOrchestrator] Starting service '${serviceId}' (attempt ${attempts + 1}/${this.config.maxRetries})`,
        );
        ...
        console.log(
          `[ServiceOrchestrator] Service '${serviceId}' is now running`,
        );
        return;
      } catch (error: any) {
        attempts++;
        console.error(
          `[ServiceOrchestrator] Failed to start '${serviceId}': ${error.message}`,
        );

        if (attempts >= this.config.maxRetries) {
          throw new Error(
            `Failed to start service '${serviceId}' after ${this.config.maxRetries} attempts: ${error.message}`,
          );
        }

        // Wait before retrying
        await new Promise((resolve) =>
          setTimeout(resolve, this.config.healthInterval),
        );
      }
    }
  }
  ...
  async startServicesForSuite(serviceIds: string[]): Promise<void> {
    console.log(
      `[ServiceOrchestrator] Starting services for suite: ${serviceIds.join(", ")}`,
    );
    ...
    if (failures.length > 0) {
      const errorMessages = failures
        .map(
          ({ serviceId, result }) =>
            `${serviceId}: ${(result as PromiseRejectedResult).reason}`,
        )
        .join("; ");
  ...
  async stopUnusedServices(keepRunning: string[] = []): Promise<void> {
    ...
    console.log(
      `[ServiceOrchestrator] Stopping unused services (keeping: ${Array.from(keepSet).join(", ")})`,
    );
    ...
    try {
      ...
      console.log(
        `[ServiceOrchestrator] Stopping services: ${servicesToStop.join(", ")}`,
      );
      ...
      console.error(
        `[ServiceOrchestrator] Failed to stop unused services: ${error.message}`,
      );
    }
  }
  ...
  async manageVRAM(preserveEmbedding: boolean = true): Promise<void> {
    console.log(
      `[ServiceOrchestrator] Managing VRAM (preserveEmbedding: ${preserveEmbedding})`,
    );
    ...
    try {
      ...
      const modelsToUnload = loadedModels.filter((model) => {
        ...
        if (preserveEmbedding) {
          ...
          if (isEmbedding) {
            console.log(
              `[ServiceOrchestrator] Preserving embedding model: ${model.name}`,
            );
            return false;
          }
        }
      ...
      console.log(
        `[ServiceOrchestrator] Unloading models: ${modelsToUnload.map((m) => m.name).join(", ")}`,
      );
      ...
      for (const model of modelsToUnload) {
        try {
          ...
          console.error(
            `[ServiceOrchestrator] Failed to unload ${model.name}: ${error.message}`,
          );
        }
      }
    } catch (error: any) {
      console.error(
        `[ServiceOrchestrator] Failed to manage VRAM: ${error.message}`,
      );
    }
  }
  ...
  async cleanup(): Promise<void> {
    const servicesToStop = Array.from(this.startedServices).filter(
      (id) => !this.config.embeddingHostServices.includes(id),
    );
    ...
    console.log(
      `[ServiceOrchestrator] Cleaning up started services: ${servicesToStop.join(", ")}`,
    );
