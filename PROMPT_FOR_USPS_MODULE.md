...
2.  **District Lookup**: The USPS API _does not_ return the Congressional District. You must implement a bridge service using the **US Census Bureau Geocoder API** (Free) to take the validated address and retrieve the `State FIPS` and `Congressional District FIPS` codes.
