...
export const ResourceManager = memo(function ResourceManager({
  onUnloadModel,
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!summaryRes.ok) {
        ...
        throw new Error(
          `Failed to fetch summary (${summaryRes.status}): ${errorText}`,
        );
      }
      ...
      if (!settingsRes.ok) {
        ...
        throw new Error(
          `Failed to fetch settings (${settingsRes.status}): ${errorText}`,
        );
      }
  ...
  const handleUnloadOllamaModel = async (modelName: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          `Failed to unload model "${modelName}" (${response.status}): ${errorText}`,
        );
        throw new Error(
          `Failed to unload model: ${response.status} ${response.statusText}`,
        );
      }
  ...
  const usedPercent =
    gpu && gpu.total_mb > 0 ? (gpu.used_mb / gpu.total_mb) * 100 : 0;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {gpu
              ? `${formatBytes(gpu.used_mb)} / ${formatBytes(gpu.total_mb)}`
              : "N/A"}
          </Typography>
        </Box>
      </AccordionSummary>
