...
interface CongressionalChatProps {
  askQuestion: (request: { message: string; member_filter?: string }) => Promise<{
    answer: string;
          ...
          >
            ...
            <Box sx={{ mt: 1, textAlign: 'center' }}>
              <Chip
                ...
                size="small"
                onClick={() => setInput("What are Greene's and Omar's positions on immigration?")}
                sx={{ m: 0.5, cursor: 'pointer' }}
              />
              <Chip
                label="How do they differ on healthcare?"
                size="small"
                onClick={() => setInput('How do Greene and Omar differ on healthcare policy?')}
                sx={{ m: 0.5, cursor: 'pointer' }}
              />
              <Chip
                label="Compare their voting records"
                size="small"
                onClick={() => setInput('Compare the voting records of Greene and Omar on recent bills.')}
                sx={{ m: 0.5, cursor: 'pointer' }}
              />
            </Box>
          </Box>
                        ...
                        {msg.sources.map((source, idx) => (
                          <Box
                            ...
                            sx={{
                              mb: 1,
                              pb: 1,
                              borderBottom: idx < msg.sources!.length - 1 ? 1 : 0,
                              borderColor: 'divider',
                            }}
                          >
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                              <Chip
                                label={source.party}
                                size="small"
                                color={source.party === 'R' ? 'error' : 'primary'}
                              />
                            ...
                            {source.url && (
                              ...
                              >
                                ...
                                <Typography variant="caption">View source</Typography>
                              </Link>
                            )}
                          </Box>
                        ))}
