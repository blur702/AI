...
const ModelsPage = memo(function ModelsPage() {
  ...
  const [selectedModel, setSelectedModel] =
    useState<OllamaModelDetailed | null>(null);
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case "parameters": {
          ...
          return (
            parseParams(b.parameters || "") - parseParams(a.parameters || "")
          );
        }
        case "vram":
  ...
  const vramUsage = useMemo(() => {
    ...
    const loadedVram = loadedModels.reduce(
      (sum, m) => sum + m.estimated_vram_mb,
      0,
    );
    ...
    return {
      ...
      percent: usedPercent,
      status:
        usedPercent < 50 ? "success" : usedPercent < 80 ? "warning" : "error",
    };
  ...
  const handleDownloadModel = async () => {
    ...
    setNotification({
      ...
      message: result.success
        ? `Download started for "${modelName}"`
        : result.message,
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      {vramUsage && (
        <Card sx={{ mb: 3 }}>
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary">
              Estimated loaded models VRAM: ~
              {formatSize(vramUsage.loadedEstimate)}
            </Typography>
          </CardContent>
        </Card>
      )}
              ...
              <Card
                sx={{
                  ...
                  borderColor: loadingModels[model.name]
                    ? loadingModels[model.name].action === "load"
                      ? "info.main"
                      : "warning.main"
                    : model.is_loaded
                      ? "success.main"
                    ...
                    <Box sx={{ mt: 1 }}>
                      ...
                      <LinearProgress
                        ...
                        value={
                          vramUsage
                            ? Math.min(
                                (model.estimated_vram_mb / vramUsage.total) *
                                  100,
                                100,
                              )
                            : 0
                  ...
                  {model.is_loaded ? (
                    <Tooltip title="Unload model">
                      <IconButton
                        ...
                        color="warning"
                        onClick={() =>
                          handleUnloadModel(model.name, model.estimated_vram_mb)
                        }
                        disabled={
                    ...
                    <Tooltip title="Load model">
                      <IconButton
                        ...
                        color="success"
                        onClick={() =>
                          handleLoadModel(model.name, model.estimated_vram_mb)
                        }
                        disabled={
                          actionLoading[model.name] ||
                          !!downloadingModels[model.name] ||
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>
            Enter the model name to download from Ollama. Examples: llama3.2,
            qwen2.5:32b, mistral:7b-instruct
          </DialogContentText>
          <TextField
            autoFocus
            fullWidth
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to remove "{modelToRemove}"? This will delete
            the model from disk and cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
        ...
        <DialogContent>
          {selectedModel && (
            <Box>
              ...
              <Typography gutterBottom>
                {selectedModel.quantization || "N/A"}
              </Typography>
              ...
              <Typography gutterBottom>
                {selectedModel.size} ({selectedModel.size_gb.toFixed(2)} GB)
              </Typography>
              ...
              <Typography gutterBottom>
                ~{formatSize(selectedModel.estimated_vram_mb)}
              </Typography>
              ...
              <Typography>
                {selectedModel.capability_description ||
                  "No description available."}
              </Typography>
            </Box>
          )}
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography variant="body2" paragraph>
            Loading a model puts it into GPU VRAM for fast inference. Unloading
            removes it from VRAM but keeps it on disk. Only loaded models can be
            used for generation.
          </Typography>
          ...
          <Typography variant="body2" paragraph>
            Each model requires VRAM when loaded. Monitor the VRAM usage bar at
            the top. If you run out of VRAM, unload models you're not using.
          </Typography>
          ...
          <Typography variant="body2" paragraph>
            Models use the format "name:tag" (e.g., "llama3.2:latest"). Common
            tags include version numbers, sizes (7b, 13b, 70b), and variants
            (instruct, chat).
          </Typography>
          ...
          <Typography variant="body2" paragraph>
            Quantization reduces model size and VRAM usage. Q4 is
            smallest/fastest, Q8 is larger/more accurate, FP16 is full
            precision. Most models use Q4_K_M for good balance.
          </Typography>
        </DialogContent>
        <DialogActions>
