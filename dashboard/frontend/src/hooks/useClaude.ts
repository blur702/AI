...
export function useClaude(): UseClaudeReturn {
  ...
  const fetchSessionOutput = useCallback(async (sessionId: string) => {
    try {
      const response = await fetch(
        `${getApiBase()}/api/claude/sessions/${sessionId}?include_output=true`,
        {
          credentials: "include",
        },
      );
  ...
  const executeNormal = useCallback(
    async (prompt: string): Promise<ClaudeExecuteResponse> => {
      setError(null);
  ...
  const executeYolo = useCallback(
    async (prompt: string): Promise<ClaudeExecuteResponse> => {
      ...
      try {
        const response = await fetch(
          `${getApiBase()}/api/claude/execute-yolo`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
  ...
  const cancelSession = useCallback(
    async (sessionId: string): Promise<boolean> => {
      try {
        const response = await fetch(
          `${getApiBase()}/api/claude/sessions/${sessionId}/cancel`,
          {
            method: "POST",
            credentials: "include",
          },
