...
const services = [
  ...
  { name: 'Stable Audio', url: 'http://localhost:7873', expect: 'Stable Audio' },
];
...
async function testService(browser, service) {
  ...
  try {
    ...
    if (response.status() !== 200) {
      ...
      return { name: service.name, status: 'FAIL', error: `HTTP ${response.status()}` };
    }
    ...
    const hasExpected = title.includes(service.expect) || content.includes(service.expect);

    if (hasExpected) {
      console.log(`  [PASS] ${service.name}: Page loaded correctly`);
      return { name: service.name, status: 'PASS', title };
    } else {
      console.log(`  [WARN] ${service.name}: Page loaded but expected content not found`);
...
async function testOllamaGeneration() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { name: 'Ollama Generation', status: 'FAIL', error: `HTTP ${response.status}` };
    }
    ...
    if (data.response) {
      ...
      return { name: 'Ollama Generation', status: 'PASS', response: data.response };
    } else {
      console.log(`  [FAIL] Ollama: No response in data`);
      return { name: 'Ollama Generation', status: 'FAIL', error: 'No response' };
    }
  } catch (error) {
...
async function testOpenWebUIChat(browser) {
  ...
  try {
    ...
    if (modelSelector) {
      console.log('  [PASS] Open WebUI: Chat interface loaded with model selector');
    ...
    console.log('  [WARN] Open WebUI: Page loaded but chat interface not fully detected');
