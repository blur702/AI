    ...
    if args.command == "scrape":
        ...
        with WeaviateConnection() as client:
            if not client.collections.exists(VSCODE_DOCS_COLLECTION):
                logger.error("Collection '%s' does not exist", VSCODE_DOCS_COLLECTION)
                sys.exit(1)

            collection = client.collections.get(VSCODE_DOCS_COLLECTION)
            response = collection.aggregate.over_all(total_count=True)
            logger.info("Collection: %s", VSCODE_DOCS_COLLECTION)
        ...
        with WeaviateConnection() as client:
            if client.collections.exists(VSCODE_DOCS_COLLECTION):
                ...
                logger.info("Deleted collection: %s", VSCODE_DOCS_COLLECTION)
            else:
                logger.info("Collection '%s' does not exist", VSCODE_DOCS_COLLECTION)
