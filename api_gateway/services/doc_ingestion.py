...
from .weaviate_connection import DOCUMENTATION_COLLECTION_NAME, WeaviateConnection
        ...
        while retries > 0:
            ...
            try:
                ...
                if inserted_count % 50 == 0:
                    ...
                    emit_progress(
                        "indexing", processed_files, total_files, f"Indexed {inserted_count} chunks"
                    )
                break
            except Exception as exc:  # noqa: BLE001
                retries -= 1
                if retries == 0:
                    errors += 1
                    logger.warning(
                        "Failed to insert chunk '%s' after retries: %s", chunk.title[:50], exc
                    )
                else:
...
def verify_files_by_path(
    ...
    if not client.collections.exists(DOCUMENTATION_COLLECTION_NAME):
        ...
        return dict.fromkeys(file_paths, 0)
