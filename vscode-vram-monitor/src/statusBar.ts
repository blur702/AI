...
export class StatusBarManager implements vscode.Disposable {
    ...
    constructor(private readonly configManager: ConfigManager) {
        this.outputChannel = vscode.window.createOutputChannel('VRAM Monitor Status');
    ...
    subscribe(monitor: VRAMMonitor): void {
        ...
        this.dataSubscription = monitor.onDataChanged((data) => this.updateStatusBar(data));
    ...
    private updateStatusBar(data: VRAMData): void {
        ...
        if (!data || !data.gpu) {
            this.applyState('GPU: N/A', 'No GPU detected or nvidia-smi unavailable', undefined);
            return;
        }

        const gpuList: GPUInfo[] = Array.isArray(data.gpu.gpus) ? data.gpu.gpus : [];
        ...
        const tooltipLines = this.buildTooltipLines(data, gpuList, usedMb, totalMb, percent, utilization, config);
        const tooltip = tooltipLines.join('\n');
        const backgroundColor = getColorForUtilization(utilization !== 0 ? utilization : percent);
    ...
    private buildTooltipLines(
        ...
        if (config.showLoadedModels) {
            lines.push(`Loaded Models: ${models.length > 0 ? models.length : 'None'}`);
        }

        const processes = Array.isArray(data.gpu_processes) ? data.gpu_processes : [];
        if (config.showGPUProcesses) {
            lines.push(`GPU Processes: ${processes.length > 0 ? processes.length : 'None'}`);
        }
    ...
    private applyState(text: string, tooltip: string, color: vscode.ThemeColor | undefined): void {
        const colorId = color?.id;
        const shouldLog = text !== this.lastText || tooltip !== this.lastTooltip || colorId !== this.lastColorId;
...
function calculatePercent(used: number, total: number, fallback: number): number {
    if (total > 0) {
