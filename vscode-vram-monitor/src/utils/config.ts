...
export class ConfigManager implements vscode.Disposable {
  ...
  private loadConfiguration(): VRAMMonitorConfig {
    ...
    const pythonPathRaw =
      settings.get<string>("pythonPath", "python")?.trim() ?? "python";
    const vramManagerRaw =
      settings
        .get<string>("vramManagerPath", "${workspaceFolder}/vram_manager.py")
        ?.trim() ?? "${workspaceFolder}/vram_manager.py";
    const pollIntervalRaw = settings.get<number>("pollInterval", 3000);
    const ollamaUrl =
      settings.get<string>("ollamaUrl", "http://localhost:11434") ??
      "http://localhost:11434";
    const ollamaLoadTimeoutRaw = settings.get<number>(
      "ollamaLoadTimeout",
      60000,
    );
    const ollamaHealthCheckTimeoutRaw = settings.get<number>(
      "ollamaHealthCheckTimeout",
      5000,
    );

    const pythonPath = this.resolvePath(pythonPathRaw, "python");
    const vramManagerPath = this.resolvePath(
      vramManagerRaw,
      "${workspaceFolder}/vram_manager.py",
    );
    const pollInterval = this.clampNumber(pollIntervalRaw, 3000, 1000);
    const ollamaLoadTimeout = this.clampNumber(
      ollamaLoadTimeoutRaw,
      60000,
      5000,
    );
    const ollamaHealthCheckTimeout = this.clampNumber(
      ollamaHealthCheckTimeoutRaw,
      5000,
      1000,
    );
    ...
    if (!fs.existsSync(vramManagerPath)) {
      this.outputChannel.appendLine(
        `[${new Date().toISOString()}] Warning: vram_manager.py not found at ${vramManagerPath}`,
      );
    }
  ...
  private clampNumber(
    ...
    if (clamped !== normalized) {
      this.outputChannel.appendLine(
        `[${new Date().toISOString()}] Warning: Clamped value from ${normalized} to ${clamped}`,
      );
    }
    return clamped;
  }
  ...
  private logConfigChanges(
    previous: VRAMMonitorConfig,
    current: VRAMMonitorConfig,
  ): void {
    ...
    if (changes.length > 0) {
      this.outputChannel.appendLine(
        `[${new Date().toISOString()}] Configuration changed:`,
      );
