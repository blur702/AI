import { VRAMData, GPUData, OllamaModel, GPUProcess, EMPTY_VRAM_DATA, isGPUData, isOllamaModel, isGPUProcess } from '../types/vramTypes';
import { PythonPaths, ExecutionOptions, fetchVRAMDataRaw, PythonNotFoundError, VRAMManagerNotFoundError, ExecutionTimeoutError, PythonBridgeError } from './pythonBridge';

export type FetchErrorType = 'python_not_found' | 'vram_manager_not_found' | 'timeout' | 'parse_error' | 'execution_error';
 ...
 */
export async function fetchVRAMData(paths: PythonPaths, options?: ExecutionOptions): Promise<FetchResult> {
    try {
