...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    try {
        ...
        vscode.window.showWarningMessage(`VRAM Monitor: Failed to initialize - ${errorMessage}`);
    }
    ...
    try {
        ...
        if (healthy) {
            ...
            console.warn('Ollama service is not available - model management features will be limited');
        }
    ...
    if (vramMonitor && ollamaService) {
        dashboardProvider = new DashboardProvider(context, vramMonitor, ollamaService, configManager);
    ...
    const refreshStatsCommand = vscode.commands.registerCommand('vramMonitor.refreshStats', async () => {
        if (vramMonitor) {
            await vramMonitor.refresh();
            const data = vramMonitor.getData();
            if (data.gpu) {
                const modelCount = Array.isArray(data.loaded_models) ? data.loaded_models.length : 0;
                vscode.window.showInformationMessage(
                    `VRAM: ${data.gpu.used_mb}/${data.gpu.total_mb}MB used (${modelCount} models loaded)`
                );
            } else {
                vscode.window.showWarningMessage('VRAM Monitor: No GPU data available');
            }
        } else {
            vscode.window.showWarningMessage('VRAM Monitor: Service not initialized');
        }
    });

    const toggleStatusBarCommand = vscode.commands.registerCommand('vramMonitor.toggleStatusBar', () => {
        if (!statusBarManager) {
            vscode.window.showWarningMessage('VRAM Monitor: Status bar is not available yet');
            return;
        }
    ...
    const showOutputCommand = vscode.commands.registerCommand('vramMonitor.showOutput', () => {
        vramMonitor?.showOutput();
    });

    const showDiagnosticsCommand = vscode.commands.registerCommand('vramMonitor.showDiagnostics', () => {
        if (vramMonitor) {
            const diagnostics = vramMonitor.getDiagnostics();
            const outputChannel = vscode.window.createOutputChannel('VRAM Monitor Diagnostics');
            outputChannel.appendLine('=== VRAM Monitor Diagnostics ===');
            outputChannel.appendLine(`Is Polling: ${diagnostics.isPolling}`);
            outputChannel.appendLine(`Poll Interval: ${diagnostics.pollInterval}ms`);
            outputChannel.appendLine(
                `Last Poll Time: ${diagnostics.lastPollTime ? new Date(diagnostics.lastPollTime).toISOString() : 'Never'}`
            );
            outputChannel.appendLine(`Last Poll Duration: ${diagnostics.lastPollDuration}ms`);
            outputChannel.appendLine(`Last Error: ${diagnostics.lastError || 'None'}`);
            outputChannel.appendLine('');
            outputChannel.appendLine('=== Configuration ===');
            outputChannel.appendLine(`Python Path: ${diagnostics.config.pythonPath}`);
            outputChannel.appendLine(`VRAM Manager Path: ${diagnostics.config.vramManagerPath}`);
            outputChannel.appendLine('');
            outputChannel.appendLine('=== Cached Data ===');
            outputChannel.appendLine(JSON.stringify(diagnostics.cachedData, null, 2));
            outputChannel.show();
        } else {
            vscode.window.showWarningMessage('VRAM Monitor: Service not initialized');
        }
    ...
    const showDashboardCommand = vscode.commands.registerCommand('vramMonitor.showDashboard', () => {
    ...
    const loadModelCommand = vscode.commands.registerCommand('vramMonitor.loadModel', async () => {
        if (!ollamaService) {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            vscode.window.showErrorMessage(`Failed to list Ollama models: ${message}`);
            return;
        }
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Model ${quickPick.label} loading initiated`);
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            vscode.window.showErrorMessage(`Failed to load ${quickPick.label}: ${message}`);
        }
    });

    const unloadAllModelsCommand = vscode.commands.registerCommand('vramMonitor.unloadAllModels', async () => {
        if (!ollamaService) {
            vscode.window.showWarningMessage('Ollama service is not configured');
            return;
        }
    ...
    const showOllamaDiagnosticsCommand = vscode.commands.registerCommand(
        ...
        async () => {
            ...
            try {
                ...
                const message = error instanceof Error ? error.message : String(error);
                vscode.window.showErrorMessage(`Failed to retrieve available models: ${message}`);
                return;
            }
            ...
            try {
                ...
                const message = error instanceof Error ? error.message : String(error);
                vscode.window.showErrorMessage(`Failed to retrieve loaded models: ${message}`);
                return;
            }

            const diagnostics = vscode.window.createOutputChannel('Ollama Service Diagnostics');
            const healthy = await ollamaService.checkHealth();
            ...
            diagnostics.appendLine(`Health Status: ${healthy ? 'Connected' : 'Disconnected'}`);
            ...
            loaded.forEach((model) =>
                diagnostics.appendLine(`  - ${model.name} (${formatBytes(model.size_vram)} VRAM)`)
            );
            ...
            diagnostics.appendLine(`Health Check Timeout: ${config.healthCheckTimeout}ms`);
            diagnostics.appendLine('');
            diagnostics.appendLine(`Last Error: ${ollamaService.getLastError() ?? 'None'}`);
            diagnostics.show();
        }
    );
