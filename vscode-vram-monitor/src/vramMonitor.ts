...
export class VRAMMonitor implements vscode.Disposable {
  ...
  private setupConfigurationWatcher(): void {
    this.configChangeSubscription = this.configManager.onDidChangeConfiguration(
      (event) => {
        const relevant =
  ...
  private async poll(): Promise<void> {
    ...
    if (this.lastPollDuration > this.pollInterval) {
      this.log(
        `WARNING: Poll took ${this.lastPollDuration}ms, longer than interval ${this.pollInterval}ms`,
      );
    }
  }
  ...
  private handleFetchResult(result: FetchResult): void {
    if (result.success) {
      ...
      if (
        result.errorType === "python_not_found" &&
        !this.hasShownPythonWarning
      ) {
      ...
      if (
        result.errorType === "vram_manager_not_found" &&
        !this.hasShownVRAMManagerWarning
      ) {
