...
export class OllamaService implements vscode.Disposable {
  ...
  constructor(
    ...
    this.configWatcher = this.configManager.onDidChangeConfiguration(
      (event) => {
        if (
  ...
  async loadModel(
    ...
    try {
      ...
      throw new ModelLoadError(modelName, {
        cause: error instanceof Error ? error : undefined,
      });
    }
  }
  ...
  async unloadModel(modelName: string, timeout?: number): Promise<boolean> {
    ...
    try {
      ...
      throw new ModelUnloadError(modelName, {
        cause: error instanceof Error ? error : undefined,
      });
    }
  }
  ...
  async unloadAllModels(): Promise<number> {
    ...
    for (const model of models) {
      try {
        ...
        this.log(
          `Failed to unload ${model.model}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
