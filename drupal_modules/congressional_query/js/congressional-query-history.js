...
(function ($, Drupal, once) {
  ...
  Drupal.behaviors.congressionalQueryHistory = {
    attach: function (context, settings) {
      const $page = $(
        once("query-history", ".congressional-query-history", context),
      );
      ...
      $page.on("click", ".delete-link", function (e) {
        ...
        if (
          !confirm(
            Drupal.t("Are you sure you want to delete query #@id?", {
              "@id": queryId,
            }),
          )
        ) {
        ...
        if (selectedCount > 0) {
          if ($bulkActions.length === 0) {
            ...
            $bulkActions.find(".bulk-delete").on("click", function () {
              if (
                !confirm(
                  Drupal.t("Are you sure you want to delete @count queries?", {
                    "@count": selectedCount,
                  }),
                )
              ) {
              ...
              ids.forEach(function (id) {
                $page
                  .find('.delete-link[data-query-id="' + id + '"]')
                  .trigger("click");
              });
            });
          } else {
            $bulkActions
              .find(".selected-count")
              .text(selectedCount + " " + Drupal.t("selected"));
          }
        } else {
          $bulkActions.remove();
        }
