...
(function ($, Drupal, drupalSettings, once) {
  ...
  Drupal.behaviors.congressionalChat = {
    attach: function (context, settings) {
      const elements = once('congressional-chat', '.congressional-chat-container', context);
      ...
      function setupEventListeners() {
        ...
        chatContainer.on('click', '.copy-answer-btn', function (e) {
          e.preventDefault();
          const content = $(this).data('content') || $(this).closest('.chat-message').find('.message-content').text();
        ...
        chatContainer.on('click', '.new-conversation-btn', function (e) {
          e.preventDefault();
          if (confirm(Drupal.t('Start a new conversation? This will clear the current chat.'))) {
      ...
      function sendMessageStreaming(message) {
        ...
        fetch(streamUrl, {
              ...
              if (line.startsWith('data: ')) {
                try {
                  ...
                  if (data.type === 'chunk') {
                    ...
                    finalizeStreamingMessage(assistantMessageEl, data.sources, data.model);
                    enableInput();
          ...
          hideTypingIndicator();
          addErrorMessage('Failed to connect. Falling back to standard mode.');
          // Fallback to AJAX
          sendMessageAjax(message);
        });
      }
      ...
      function sendMessageAjax(message) {
        $.ajax({
          ...
          success: function (response) {
            ...
            addMessage('assistant', response.answer, response.sources, response.model);
            enableInput();
            announceToScreenReader('Response received');
          },
          error: function (xhr) {
      ...
      function updateStreamingMessage(messageEl, content) {
        ...
        contentEl.html(escapeHtml(content) + '<span class="streaming-cursor">|</span>');
        scrollToBottom();
      }
      ...
      function finalizeStreamingMessage(messageEl, sources, model) {
        ...
        const metaEl = $('<div>')
          ...
          .html('<span class="model-name">' + escapeHtml(model || '') + '</span>' +
      ...
      function addMessage(role, content, sources, model) {
        const messageEl = $('<div>')
          ...
          .attr('role', 'article')
          .attr('aria-label', role === 'user' ? 'Your message' : 'Assistant response');
      ...
      function buildSourcesElement(sources) {
        ...
        const toggleBtn = $('<button>')
          ...
          .html('<span class="toggle-icon" aria-hidden="true">+</span> ' + sources.length + ' sources');
        ...
        sources.forEach(function (source) {
          ...
          const header = $('<div>')
            ...
            .html('<span class="member-chip ' + partyClass + '">' +
                  escapeHtml(source.member_name) + ' (' + escapeHtml(source.party || '?') + '-' + escapeHtml(source.state || '??') + ')' +
                  '</span>');
          ...
          if (source.url) {
            const link = $('<a>')
              ...
              .attr('aria-label', 'View source: ' + (source.title || 'Untitled') + ' (opens in new tab)')
              .html('View source <span class="external-icon" aria-hidden="true">&#8599;</span>');
      ...
      function editMessage(messageIndex) {
        const messageEl = messagesContainer.find('[data-message-index="' + messageIndex + '"]');
        const contentEl = messageEl.find('.message-content');
        const currentContent = contentEl.text();

        const editForm = $('<div class="edit-form">' +
          '<textarea class="edit-input">' + escapeHtml(currentContent) + '</textarea>' +
        ...
        editForm.find('.save-edit-btn').on('click', function () {
          ...
          if (newContent && newContent !== currentContent) {
            $.ajax({
              url: '/congressional/chat/message/' + conversationId + '/' + messageIndex,
              method: 'PATCH',
              contentType: 'application/json',
              ...
              success: function () {
                ...
                if (!messageEl.find('.edited-indicator').length) {
                  contentEl.after('<span class="edited-indicator">(edited)</span>');
                }
              },
              error: function () {
          ...
          success: function () {
            ...
            messageEl.fadeOut(function () {
              ...
              messagesContainer.find('.chat-message').each(function (index) {
                ...
                $(this).find('[data-message-index]').attr('data-message-index', index);
              });
            });
      ...
      function startNewConversation() {
        $.ajax({
          ...
          success: function (response) {
            ...
            location.href = '/congressional/chat?conversation_id=' + conversationId +
              (memberFilter ? '&member_filter=' + encodeURIComponent(memberFilter) : '');
          },
          error: function () {
      ...
      function showExportOptions() {
        ...
        if (format && ['json', 'markdown', 'html'].includes(format.toLowerCase())) {
          window.location.href = exportUrl + '/' + conversationId + '/' + format.toLowerCase();
        }
      }
