...
(function ($, Drupal, drupalSettings, once) {
  ...
  Drupal.behaviors.congressionalQueryStatus = {
    attach: function (context, settings) {
      const elements = once('congressional-status', '.connection-status-block', context);
      ...
      function updateStatusDisplay(data) {
        ...
        statusBlock.find('.last-check-time').text(timestamp.toLocaleTimeString());

        // Update overall status.
        statusBlock.find('.overall-status')
      ...
      function updateServiceStatus(service, status) {
        ...
        serviceEl.find('.status-indicator')
        ...
        if (detailsEl.length > 0 && status.details) {
          ...
          for (const key in status.details) {
            detailsHtml += '<div class="detail-item"><span class="detail-key">' +
                          escapeHtml(key) + ':</span> <span class="detail-value">' +
                          escapeHtml(String(status.details[key])) + '</span></div>';
          }
        ...
        if (service === 'weaviate' && status.details && status.details.document_count !== undefined) {
      ...
      function showNotification(message, type) {
        ...
        if (typeof Drupal.Message !== 'undefined') {
          ...
          messageArea.add(message, { type: type === 'error' ? 'error' : 'status' });
        } else {
